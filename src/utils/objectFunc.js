...
func.diff = function (objA, objB, deep = true) {
  ...
  if (typeof objA === 'function') {
    ...
    try {
      ...
      const oldProps = Object.getOwnPropertyNames(objA)
      const newProps = Object.getOwnPropertyNames(objB)
      if (oldProps.length !== newProps.length) {
        return true // 不相同
      }

      for (let i = 0; i < oldProps.length; i++) {
        const propName = oldProps[i] // 属性名
        // 值
...
func.toTreeData = function (
  ...
  try {
    ...
    const ids = new Map() // 所有id值，目的是为了找到有父节点id,但是找不到父节点的节点
    // 添加节点
    ...
    while (data.length !== nodeHash.size && count < maxnum) {
      ...
      if (item[parentField] && !nodeHash.has[item[idField]]) {
        // 还没有加入的
        const posArr = nodeHash.get(item[parentField]) // 拿出父节点的路径
        if (posArr) {
          // 说明父节点已经找到了
